...
module.exports = class UsersDBApi {
    ...
    static async findBy(where, options) {
        ...
        output.cases_owner_user = await users.getCases_owner_user({
            transaction
        });
        
        
        output.tasks_assignee_user = await users.getTasks_assignee_user({
            transaction
        });
        
        
        output.documents_uploaded_by_user = await users.getDocuments_uploaded_by_user({
            transaction
        });
        
        
        output.appeal_drafts_author_user = await users.getAppeal_drafts_author_user({
            transaction
        });
        
        
        output.notes_author_user = await users.getNotes_author_user({
            transaction
        });
        
        
        output.activity_logs_actor_user = await users.getActivity_logs_actor_user({
            transaction
        });
